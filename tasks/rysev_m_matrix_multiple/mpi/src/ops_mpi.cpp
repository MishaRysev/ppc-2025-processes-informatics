...
void RysevMMatrMulMPI::DistributeMatrixA(int rank, int size) {
    ...
    MPI_Scatterv(A_.data(), send_counts_.data(), displs_.data(), MPI_INT,
                 local_A_.data(), send_counts_[rank], MPI_INT,
                 0, MPI_COMM_WORLD);
}
...
void RysevMMatrMulMPI::GatherResults(int rank, int size) {
    ...
    if (rank == 0) {
        MPI_Gatherv(local_C_.data(), local_rows_ * sizes_.N, MPI_INT,
                    C_.data(), recv_counts.data(), recv_displs.data(), MPI_INT,
                    0, MPI_COMM_WORLD);
    } else {
        MPI_Gatherv(local_C_.data(), local_rows_ * sizes_.N, MPI_INT,
