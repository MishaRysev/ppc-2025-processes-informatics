...
class MaxAdjacentDiffFuncTest : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    return expected_output_.first == output_data.first && 
           expected_output_.second == output_data.second;
  }
...
const std::array<TestType, 10> kTestParams = {
    std::make_tuple(1, "increasing"),
    std::make_tuple(2, "single_jump"),
    std::make_tuple(3, "decreasing"),
    std::make_tuple(4, "negative"),
    std::make_tuple(5, "alternating"),
    std::make_tuple(6, "minimal"),
    std::make_tuple(7, "constant"),
    std::make_tuple(8, "max_in_middle"),
    std::make_tuple(9, "max_at_end"),
    std::make_tuple(10, "multiple_max")
};

const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<RysevMMaxAdjacentDiffMPI, InType>(kTestParams, PPC_SETTINGS_example_processes),
